...
namespace swiftnav_piksi{
    ...
    void PIKSI::setMetricType(int metricType){
    ...
    void PIKSI::setTime(std_msgs::Time time){
        this->time = time;
    }

    bool PIKSI::isSendingData(){
        return this -> sendingData;
    }
    void PIKSI::setSendingData(bool sendingData){
        this -> sendingData = sendingData;
    }

    bool PIKSI::PIKSIOpen( ){
        boost::mutex::scoped_lock lock( cmd_lock );
        this -> isConnected = PIKSIOpenNoLock();
    ...
    bool PIKSI::PIKSIOpenNoLock(){
        ...
        rtkA_pub = nh.advertise<swiftnav_piksi::rtk>( "gps/rtk", 1 );
    ...
    void baseline_ned_callback(u16 sender_id, u8 len, u8 msg[], void *context){
       ...
       // driver->rtk_pub.publish( rtk_odom_msg );
        ...
        pthread_mutex_unlock(&driver->_mutex);
        pthread_mutex_lock(&driver->_mutex);
        if(driver->nortD.size() == driver->n){
            driver->nortD.clear();
            driver->eastD.clear();
